...
#https://github.com/salesforce/hierarchicalContrastiveLearning/blob/master/losses/losses.py
#https://arxiv.org/pdf/2204.13207
...
def reformat(features, labels, device='cpu'):
    ...
    for label in labels:
        level_1.append(label[0])
        level_2.append(label[1])
        level_3.append(label[2])
        level_4.append(label[3])

    return features, [torch.tensor(level_1).to(device), torch.tensor(level_2).to(device), torch.tensor(level_3).to(device), torch.tensor(level_4).to(device)]
...
#generated by Google Gemini Assistant
class HiMulConLoss(nn.Module):
    def __init__(self, temperature=0.5):
        super(HiMulConLoss, self).__init__()
        self.temperature = temperature


    def forward(self, features, hierarchical_labels):
        """
        Calculates the HiMulCon loss.

        Args:
            features (torch.Tensor): The feature representations of the instances.
            hierarchical_labels (list of torch.Tensor): A list of label tensors, where each tensor 
                                                      represents labels at a specific level of the hierarchy.

        Returns:
            torch.Tensor: The HiMulCon loss.
        """
        device = 'cuda' if torch.cuda.is_available() else 'cpu'

        features, hierarchical_labels = reformat(features, hierarchical_labels, device=device)

        total_loss = 0
        for level_labels in hierarchical_labels:
            total_loss += self.contrastive_loss(features, level_labels)
        return total_loss / len(hierarchical_labels)

    def contrastive_loss(self, features, labels):
        """
        Calculates the contrastive loss for a single level of the hierarchy.

        Args:
            features (torch.Tensor): The feature representations of the instances.
            labels (torch.Tensor): The labels for the current level.

        Returns:
            torch.Tensor: The contrastive loss for the current level.
        """
        ...
        positive_mask = (labels.unsqueeze(0) == labels.unsqueeze(1)).fill_diagonal_(0)
        negative_mask = ~positive_mask

        # Compute loss
        numerator = torch.exp(similarity_matrix).masked_select(positive_mask).sum()
        denominator = torch.exp(similarity_matrix).masked_select(negative_mask).sum()
